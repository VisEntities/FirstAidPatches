...
{
	[Info("MasterLock", "S0N_0F_BISCUIT", "1.0.3")]
	...
	class MasterLock : RustPlugin
	{
		...
		{
			public uint seed = 0;
			public Dictionary<ulong, bool> buildings { get; set; } = new Dictionary<ulong, bool>();
		}
		...
		private void FindBuildingPrivileges()
		{
			...
			foreach (ulong id in data.buildings.Keys)
			{
                NetworkableId networkableId = new NetworkableId(id);
                BaseNetworkable networkable = BaseNetworkable.serverEntities.Find(networkableId);
		...
		void ToggleMasterLock(BasePlayer player, string command, string[] args)
		{
			...
			if (data.buildings.ContainsKey(privilege.net.ID.Value))
			{
				if (data.buildings[privilege.net.ID.Value])
				{
					data.buildings[privilege.net.ID.Value] = false;
					player.ChatMessage(Lang("Disabled", player.UserIDString));
				}
				else
				{
					data.buildings[privilege.net.ID.Value] = true;
			...
			{
				data.buildings.Add(privilege.net.ID.Value, true);
				InitializeMasterLock(privilege);
				player.ChatMessage(Lang("Enabled", player.UserIDString));
			}
			SaveData();
		...
		void CanChangeCode(CodeLock codeLock, BasePlayer player, string newCode, bool isGuestCode)
		{
			if (codeLock.GetParentEntity() is BuildingPrivlidge)
			{
				...
				if (data.buildings.ContainsKey(privilege.net.ID.Value))
				{
					if (data.buildings[privilege.net.ID.Value] && !isGuestCode)
					{
						uint count = UpdateCode(privilege, newCode);
						player.ChatMessage(Lang("CodeUpdate", player.UserIDString, count));
					}
					else if (!data.buildings[privilege.net.ID.Value] && !isGuestCode && (bool)Config["Display Tooltips"])
