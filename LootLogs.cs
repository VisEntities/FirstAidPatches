...
{
    [Info("LootLogs", "k1lly0u", "0.1.5", ResourceId = 2065)]
    [Description("Logs all items deposited and removed from storage, stash and oven containers")]
    class LootLogs : RustPlugin
    {
        ...
        void Log(string playername, string item, string type, string id, string entityname, bool take)
        {
            ...
            LogToFile($"{type}-{entityname}-{id}", $"[{dateTime}] {playername} {taketype} {item}", this);
        }
        void DeathLog(string type, string entityname, string id, string position, string killer)
        {
            var dateTime = DateTime.Now.ToString("yyyy-MM-dd");
            LogToFile($"{type}-deaths", $"[{dateTime}] Name:{entityname} | BoxID:{id} | Position:{position} | Killer: {killer} | LogFile: oxide/logs/LootLogs/lootlogs_{type}-deaths_xxxxxxxx.txt", this);
            LogToFile($"{type}-deaths", "-------------------------------------------------------------------------", this);
        }
        #endregion
